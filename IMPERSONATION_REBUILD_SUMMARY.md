# 🎸 RAIOPS Multi-RDS Impersonation Rebuild - COMPLETE
...
- ✅ Deleted `tenant_master` id=2 from RAIOPS (duplicate Link Restaurant Group on RDS2)
...
- ✅ Fixed RAIOPS's `rds_instances` table - changed RDS1 `rai_database` from `linkrg_prod_test` to `linkrg`

### Phase 2: Ghost User Sync Mechanism
- ✅ Created new artisan command: `php artisan sync:ghost-users`
- ✅ Location: `/var/www/html/raiops/app/Console/Commands/SyncGhostUsers.php`
- ✅ Ghost users created on both RDS instances for all 3 RAIOPS admins:
  - RDS1 (localhost): ids 92, 93, 94
  - RDS2 (AWS): ids 176, 177, 178

### Phase 3: Simplified RAI RaiOpsImpersonationController
- ✅ Rewrote `/var/www/html/rai/app/Http/Controllers/RaiOpsImpersonationController.php`
- ✅ Key simplifications:
  - Ghost users are PRE-CREATED (not created on-the-fly)
  - Finds ghost user by `raiops_admin_id` + `is_ghost_admin`
...
### Ghost Users:
- RDS1: ids 92, 93, 94 (for RAIOPS admins 1, 2, 3)
- RDS2: ids 176, 177, 178 (for RAIOPS admins 1, 2, 3)
...
```
1. RAIOPS Admin clicks "Impersonate" on tenant
2. RAIOPS generates JWT with: raiops_admin_id, remote_tenant_id, rds_instance_id
3. Redirect to: rai.test/raiops-impersonate?token=xxx
4. RAI validates JWT
5. RAI switches to target RDS
6. RAI finds PRE-CREATED ghost user by raiops_admin_id
...
### Sync Ghost Users (run from RAIOPS)
```bash
...
- **REWRITTEN**: `app/Http/Controllers/RaiOpsImpersonationController.php` - Simplified flow
- **UPDATED**: `.env` - Added RAIOPS_IMPERSONATION_SECRET
...
Generate a test URL from RAIOPS:
```bash
...
1. **Test in browser** - Actually click through the RAIOPS UI to impersonate
2. **Verify dashboard loads** - Make sure tenant data shows correctly
3. **Test "Return to RAIOPS"** - The exit flow
4. **Add ghost user sync to deploy** - Run `sync:ghost-users` when new RAIOPS admins are created
